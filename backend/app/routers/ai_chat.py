...
from app.auth import require_admin, require_teacher
...
async def ai_chat(
    ...
    try:
        ...
        sql = await question_to_sql(data.question, user.school_id, history)

        # Execute the AI-generated SQL safely
        result = await db.execute(text(sql))
        rows = [dict(r._mapping) for r in result.fetchall()]

        answer = await format_answer(data.question, rows, sql, history)
